...
String readMQ2Value() {
  ...
  float percentage = ((float)(mq2Value - 0) / (4095 - 0)) * 100.0;
  return String("MQ2 value: ") + String(percentage, 2) + "%";
}
...
String readLocation(double latitude, double longitude) {
  String locationString = String(latitude, 6) + ", " + String(longitude, 6);
...
String readTime(const TinyGPSDate &date, const TinyGPSTime &time) {
  ...
  String timeString =  String(day) + "-" + String(month) + "-" + String(year) + " " + String(hour) + ":" + String(localTime.minute()) + ":" + String(localTime.second());
